...
import { Item, ItemId } from '../../../items/item';
import { damageType } from '../../../items/weapon/damage';
...
import {
  ...
  equipZones,
  majorBodyParts,
} from '../../../items/doll/types';
import { DataStore } from '../../../../engine/models/store/store';
import { BaseEntityModel } from '../../../engine/models/entity/models/entity';

export class Doll extends BaseEntityModel {
  private bodyParts: Record<equipZones | battleZones, DollBodyPart>;
  private equippedItems: Record<ItemId, Item> = {};
  private character: Character;

  constructor(character: Character, store: DataStore) {
    super(store, ['doll']);
    this.character = character;
    this.bodyParts = {} as unknown as Record<equipZones | battleZones, DollBodyPart>;
  ...
  recordEquippedItem(item: Item) {
    if (!this.equippedItems[item.getId()]) {
      ...
      this._setValue('equippedItems', this.equippedItems);
    }
  }
  ...
  getItemsByZoneAsMap(code: equipZones): Item[] {
    return this.bodyParts[code]?.getItems() || [];
  }
